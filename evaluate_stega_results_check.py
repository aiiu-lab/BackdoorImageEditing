...
distortion_strength_paras = dict(
    compression=50,  # JPEG(Q=50)
    blurring=1,      # Gaussian Blur (std =1.0)
    noise=0.06,      # Gaussian Noise (0.06)
    crop=0.035,      # Crop (crop ratio)
    dropout=0.3,     # Dropout (pixel dropout probability)
    hue=0.2,         # Hue (normalized shift factor)
    saturation=15.0, # Saturation factor
    resize=0.7,      # Resize scale factor
...
def image_distortion(images, distortion_type):
    ...
    #     fixed_factor = random.uniform(*distortion_strength_paras["brightness"])
    #     def distort(img):
    #         enhancer = ImageEnhance.Brightness(img)
    #         return enhancer.enhance(fixed_factor)
    # elif distortion_type == "contrast":
    #     fixed_factor = random.uniform(*distortion_strength_paras["contrast"])
    #     def distort(img):
    #         enhancer = ImageEnhance.Contrast(img)
    #         return enhancer.enhance(fixed_factor)
    if distortion_type == "blurring":
        fixed_kernel = distortion_strength_paras["blurring"]
        def distort(img):
            return img.filter(ImageFilter.GaussianBlur(fixed_kernel))
    elif distortion_type == "noise":
        fixed_std = distortion_strength_paras["noise"]
        def distort(img):
            image_tensor = transforms.ToTensor()(img)
            noise = torch.randn(image_tensor.size()) * fixed_std
            noisy_tensor = (image_tensor + noise).clamp(0, 1)
            return transforms.ToPILImage()(noisy_tensor)
    elif distortion_type == "compression":
        fixed_quality = distortion_strength_paras["compression"]
        def distort(img):
            buffered = BytesIO()
            img.save(buffered, format="JPEG", quality=fixed_quality)
            buffered.seek(0)
            return Image.open(buffered)
    elif distortion_type == "crop":
        # crop a portion (factor: 1 - crop_ratio) and then resize back to original size
        crop_ratio = distortion_strength_paras.get("crop", 0.035)
        def distort(img):
            w, h = img.size
...
def log_validation_set(args, pipeline, accelerator, eval_dataset, generator, epoch, output_dir, gt_msgs, visual_ncols=4):
    ...
    for batch in eval_dataloader:
        ...
        captions_all.extend(batch["edited_image_caption"])
    ...
    # - edited_image_list 為 pipeline 產生的 edited_imgs 的 PIL 圖片
        ...
        txt_gtimgs_sim, txt_imgs_sim, sim_image, sim_gt_orig_consistency, sim_img_orig_consistency = clip_sim(original_tensors, gtedited_tensors, edited_imgs_tensors, captions_all)
    
    avg_txt_gtimgs_sim = txt_gtimgs_sim.mean().item()
    avg_txt_imgs_sim = txt_imgs_sim.mean().item()
    avg_sim_image = sim_image.mean().item()
    avg_sim_gt_orig_consistency = sim_gt_orig_consistency.mean().item()
    avg_sim_img_orig_consistency = sim_img_orig_consistency.mean().item()
    
    logger.info(f"CLIP text-gtedited_imgs similarity: {avg_txt_gtimgs_sim:.4f}")
    logger.info(f"CLIP text-edited_imgs similarity: {avg_txt_imgs_sim:.4f}")
    logger.info(f"CLIP image similarity : {avg_sim_image:.4f}")
    logger.info(f"CLIP GT Edited Image v.s Original Image similarity : {avg_sim_gt_orig_consistency:.4f}")
    logger.info(f"CLIP edited Image v.s Original Image similarity : {avg_sim_img_orig_consistency:.4f}")
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "wandb":
            tracker.log({"validation_set": wandb_table, 
                         ...
                         "CLIP_text-gtedited_imgs_similarity": avg_txt_gtimgs_sim, 
                         "CLIP_text-edited_imgs_similarity": avg_txt_imgs_sim, 
                         "CLIP_image_similarity": avg_sim_image
                         })
    ...
    metrics = {
        ...
        "CLIP_text-edited_imgs_similarity": avg_txt_imgs_sim, 
        "CLIP_text-gtedited_imgs_similarity": avg_txt_gtimgs_sim, 
        "CLIP_edited_Image_v.s_Original_Image_similarity": avg_sim_img_orig_consistency,
        "CLIP_GT_Edited_Image_v.s_Original_Image_similarity": avg_sim_gt_orig_consistency
    }
...
def parse_args():
    ...
    parser.add_argument("--ckpt_path", type=str, default="./stega_instruct-pix2pix-model/runs/Multi1_Stega_bad_0.2_combined_2025-04-01_17-17/ckpt_epoch_39", help="Path to the backdoored model.")
